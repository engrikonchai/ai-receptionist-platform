 ...
 * supplied it. Every subsequent read/write is scoped to this verified
 * id, so a spoofed id can never surface another owner's billing state.
 *
 * Deliberately duplicated from src/features/inbox/api/authorize.ts,
 * src/features/knowledge/api/authorize.ts, and
 * src/features/leads/api/authorize.ts (same logic, same messages)
