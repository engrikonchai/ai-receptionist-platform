...
/**
 * The one place every knowledge query and action funnels through
 * before touching `knowledge_items`. Re-loads the signed-in owner's
 * session and the businesses Row Level Security lets them see (never
 * the service-role key), then confirms the requested business id is
 ...
 *
 * Deliberately duplicated from src/features/inbox/api/authorize.ts
 * (same logic, same messages) rather than imported cross-feature —
 * each feature owns its own thin authorization wrapper over the shared
 * `loadOwnerContext()`, per this app's existing convention.
 */
