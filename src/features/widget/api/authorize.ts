...
/**
 * The one place every widget-settings query and action funnels through
 * before touching `businesses` or `widget_settings`. Re-loads the
 ...
 * trusted just because a caller (a cookie, a browser-supplied value)
 * supplied it. Every table read/write downstream still filters by this
 * verified id in addition to RLS, so a spoofed id can never surface
 * another owner's data even if this check were somehow bypassed.
 *
