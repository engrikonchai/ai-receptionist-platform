...
describe('verifyActiveBusiness', () => {
  ...
  it('rejects a business id that is not in the RLS-scoped businesses this owner can see — the cross-business/ownership guard, including a browser-supplied id for another owner’s business', async () => {
    vi.mocked(loadOwnerContext).mockResolvedValue({
      status: 'ok',
      user: stubUser,
      profile: {} as never,
